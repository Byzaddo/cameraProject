...
try:
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) >= 2:
            ...
            point1_screen = (int(point1_hand1.x * image_width), int(point1_hand1.y * image_height))
            point2_screen = (int(point2_hand2.x * image_width), int(point2_hand2.y * image_height))
            cv2.line(frame, point1_screen, point2_screen, (0, 255, 0), 2) 

            midpoint = (
                int((point1_screen[0] + point2_screen[0]) / 2),
                int((point1_screen[1] + point2_screen[1]) / 2),
            )
            cv2.putText(
                frame, f"{distance_cm:.2f} cm", midpoint,
